  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <p className="text-gray-400 text-sm">{properties.length} properties</p>
      ...
      {properties.length === 0 ? (
        <div className="text-center py-16 text-gray-500">
          ...
          <p className="font-medium text-gray-400 mb-1">No real estate added</p>
        ...
        <div className="space-y-3">
          {properties.map(prop => {
            ...
            return (
              <div key={prop.id} className="bg-gray-900 rounded-xl border border-gray-700 p-5">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <p className="text-white font-bold text-base">{prop.name}</p>
                    {prop.location && <p className="text-gray-400 text-sm">📍 {prop.location}</p>}
                    <p className="text-gray-500 text-xs mt-0.5">Purchased: {formatDate(prop.purchaseDate)}</p>
                  </div>
                  <div className="flex items-center gap-1">
                    <button onClick={() => setEditAsset(prop)} className="p-1.5 text-gray-500 hover:text-indigo-400 hover:bg-gray-700 rounded-lg">✏️</button>
                    <button onClick={() => handleDelete(prop.id)} className="p-1.5 text-gray-500 hover:text-red-400 hover:bg-gray-700 rounded-lg">🗑️</button>
                  </div>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  <div>
                    <p className="text-gray-500 text-xs">Purchase Price</p>
                    <p className="text-white font-medium">{formatCurrency(prop.purchasePrice)}</p>
                  </div>
                  <div>
                    <p className="text-gray-500 text-xs">Current Value</p>
                    <p className="text-white font-medium">{formatCurrency(stats.currentValue)}</p>
                  </div>
                  <div>
                    <p className="text-gray-500 text-xs">Appreciation</p>
                    <p className={`font-medium ${stats.pnl >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                      {stats.pnl >= 0 ? '+' : ''}{formatCurrency(stats.pnl)} ({formatPercent(stats.pnlPercent)})
                    </p>
                  </div>
                  <div>
                    <p className="text-gray-500 text-xs">CAGR (approx)</p>
                    <p className={`font-medium ${stats.xirr !== null && stats.xirr >= 0 ? 'text-green-400' : 'text-gray-500'}`}>
                      {formatXIRR(stats.xirr)}
                    </p>
                  </div>
                </div>
                {prop.notes && <p className="text-gray-500 text-xs mt-3 pt-3 border-t border-gray-700">{prop.notes}</p>}
              </div>
            );
          })}
        </div>
